    ...
    for (i1, p1) in path.iter().enumerate() {
        ...
        if i1 <= path.len() - 3 {
            ...
            for (i2, p2) in path[offset..].iter().enumerate() {
                let dist = get_dist(*p1, *p2);
                if dist >= 2 && dist <= 20 {
                    let win = (i2 + offset).abs_diff(i1) - dist;
                    if win >= 100 {
                        // println!("{:?},{i1} to {:?},{i2}, wins {}", p1, p2, win);
                        if let Some(prev) = result.get(&win) {
                            result.insert(win, prev + 1);
                        } else {
                            result.insert(win, 1);
                        }
                    }
...
fn get_dist(p1: (usize, usize), p2: (usize, usize)) -> usize {
    let x_dist = p1.0.abs_diff(p2.0);
    let y_dist = p1.1.abs_diff(p2.1);
    x_dist + y_dist
}
