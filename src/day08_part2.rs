...
fn add_antinodes(
    ...
    loop {
        ...
        let b2y;
        if dx < 0 {
            b1x = a1.0.min(a2.0) + mult * dx;
            b2x = a1.0.max(a2.0) - mult * dx;
        } else {
            b1x = a1.0.max(a2.0) + mult * dx;
            b2x = a1.0.min(a2.0) - mult * dx;
        }
        if dy < 0 {
            b1y = a1.1.min(a2.1) + mult * dy;
            b2y = a1.1.max(a2.1) - mult * dy;
        } else {
            b1y = a1.1.max(a2.1) + mult * dy;
            b2y = a1.1.min(a2.1) - mult * dy;
        }
        // println!(
        //     "with mult = {}, {} in {},{} and {} in {},{} create antinodes at {},{} and {},{}",
        //     mult, a1.2, a1.0, a1.1, a2.2, a2.0, a2.1, b1x, b1y, b2x, b2y
        // );
        ...
        if !(b1x >= 0 && b1x < grid.max_l as isize && b1y >= 0 && b1y < grid.max_c as isize)
            && !(b2x >= 0 && b2x < grid.max_l as isize && b2y >= 0 && b2y < grid.max_c as isize)
        {
            // println!("done with these 2");
            break;
        }
        mult += 1;
    }
